...
function dedupeSafeSpots(safeSpots: SafeSpot[]) {
  const uniqueSafeSpots = new Map<string, SafeSpot>();

  for (const safeSpot of safeSpots) {
    const coordinateKey = safeSpot.coordinates
      .map((coordinate) => coordinate.toFixed(5))
      .join(",");
    const key = `${safeSpot.name.toLowerCase()}-${coordinateKey}`;
    const existingSafeSpot = uniqueSafeSpots.get(key);

    if (!existingSafeSpot || safeSpot.distance < existingSafeSpot.distance) {
      uniqueSafeSpots.set(key, safeSpot);
    }
  }

  return Array.from(uniqueSafeSpots.values()).sort(
...
async function fetchMapboxSafeSpots(origin: Coordinates) {
  if (!MAPBOX_TOKEN) {
    return [];
  }
...
async function fetchOpenStreetMapSafeSpots(origin: Coordinates) {
  ...
  return (
    data.elements
      ...
      .filter((safeSpot): safeSpot is SafeSpot => safeSpot !== null) ?? []
  );
}

export function useSafeSpots(): SafeSpotsState {
  const [safeSpots, setSafeSpots] = useState<SafeSpot[]>([]);
  const [isLoadingSafeSpots, setIsLoadingSafeSpots] = useState(false);
  const [safeSpotsError, setSafeSpotsError] = useState<string | null>(null);

  const findSafeSpots = useCallback(async (origin: Coordinates) => {
    setIsLoadingSafeSpots(true);
    setSafeSpotsError(null);

    try {
      const [mapboxSafeSpots, openStreetMapSafeSpots] = await Promise.all([
        fetchMapboxSafeSpots(origin).catch(() => []),
        fetchOpenStreetMapSafeSpots(origin).catch(() => []),
      ]);
      const nextSafeSpots = dedupeSafeSpots([
        ...openStreetMapSafeSpots,
        ...mapboxSafeSpots,
      ]).slice(0, 20);

      setSafeSpots(nextSafeSpots);
      setSafeSpotsError(
        nextSafeSpots.length === 0
      ...
      const message =
        safeSpotError instanceof Error
          ? safeSpotError.message
          : "Could not find safe spots.";

      setSafeSpots([]);
      setSafeSpotsError(message);
      throw new Error(message);
    } finally {
      setIsLoadingSafeSpots(false);
    }
  }, []);
